/**
 * efibootmgr.c - Manipula variáveis EFI conforme exportadas em /sys/firmware/efi/
 *     efivars ou vars (anteriormente /proc/efi/vars)
 *
...
static void read_vars(char **namelist, list_t *head)
{
    ...
    for (i=0; namelist[i] != NULL; i++)
    {
        if (namelist[i])
        {
            ...
            /**
             * O firmware mais recente do XNU define um bit alto que parece inválido para esse
             * núcleo se o escrevermos de volta, então vamos zerá-lo se estiver definido, pois
...
static var_entry_t *make_var(const char *prefix, list_t *var_list)
{
    ...
    sz = make_linux_load_option(&entry->data, &entry->data_size, extra_args, extra_args_size);
...
static int read_order(const char *name, var_entry_t **order)
{
    ...
    {
        /**
         * O firmware mais recente do XNU define um bit alto que parece inválido
         * para esse núcleo se o escrevermos de volta, então vamos zerá-lo se
         * estiver definido, pois seria inválido configurá-lo de qualquer
         * maneira.
         */
...
static int add_to_order(const char *name, uint16_t num, uint16_t insert_at)
{
    ...
    /**
     * Agora temos um vetor (em order->data) do pedido. Copie todas as entradas
     * que devem preceder, adicione nossa entrada e copie o restante da matriz
     * antiga.
     */
    ...
    {
        memcpy(new_data + insert_at + 1, old_data + insert_at, order->data_size - insert_at * sizeof(uint16_t));
    }
...
static int remove_from_order(const char *name, uint16_t num)
{
    ...
    /**
     * Agora temos um vetor (em order->data) do pedido. Obter qualquer instância
     * da entrada que estamos excluindo, deslocando o restante para baixo.
     */
    ...
    /**
     * Se nada for removido, não há necessidade de atualizar a variável de pedido.
     */
    if (new_i == old_i)
    {
        /**
...
static void show_order(const char *name)
{
    ...
    /**
     * Agora temos um vetor (em order->data) do pedido. Primeiro adicione
     * nossa entrada e copie o vetor antigo.
     */
    data = (uint16_t *)order->data;
...
static int set_active_state(const char *prefix)
{
    ...
    {
        /**
         * Se chegarmos aqui, o número fornecido não foi encontrado.
         */
